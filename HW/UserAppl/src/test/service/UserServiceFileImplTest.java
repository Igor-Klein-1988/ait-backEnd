...
class UserServiceFileImplTest {
    private UserServiceFileImpl userService;
    File tempFile;

    @BeforeEach
    void setUp() {
        try {
            tempFile = File.createTempFile("temp_usersee", ".txt");
        } catch (IOException e) {
            e.printStackTrace();
        }

        userService = new UserServiceFileImpl(tempFile.getAbsolutePath());
    }
